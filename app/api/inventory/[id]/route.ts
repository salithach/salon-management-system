...
export async function PUT(
    ...
    try {
        ...
        const res = await fetch(`${API_BASE}/api/v1/inventory/${id}`, {
            method: "PUT",
        ...
        if (!res.ok) {
            return NextResponse.json({ message: extractMessage(data, "Failed to update inventory item") }, { status: res.status })
        }
...
export async function DELETE(
    ...
    try {
        ...
        if (!res.ok) {
            ...
            return NextResponse.json({ message: extractMessage(data, "Failed to delete inventory item") }, { status: res.status })
        }
